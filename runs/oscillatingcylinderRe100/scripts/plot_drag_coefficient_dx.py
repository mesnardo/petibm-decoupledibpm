"""Plot the history of the drag coefficient for different grid sizes."""
...
# Get drag coefficient obtained for grid size 1024x1024.
label = r'$1024 \times 1024$'
filepath = rootdir / 'algo1-1024x1024' / 'forces-0.txt'
t, cd = get_drag_coefficient(filepath)
simus[label] = dict(t=f * t, cd=cd,
                    kwargs=dict(color='black', linestyle=':', zorder=1))

# Get drag coefficient obtained for grid size 512x512.
label = r'$512 \times 512$'
filepath = rootdir / 'algo1' / 'forces-0.txt'
t, cd = get_drag_coefficient(filepath)
simus[label] = dict(t=f * t, cd=cd,
                    kwargs=dict(color='black', linestyle='--', zorder=2))

# Get drag coefficient obtained for grid size 256x256.
label = r'$256 \times 256$'
filepath = rootdir / 'algo1-256x256' / 'forces-0.txt'
t, cd = get_drag_coefficient(filepath)
simus[label] = dict(t=f * t, cd=cd,
...
filepath = figdir / 'drag_coefficient_dx.png'
